/*
 * A demo of a program which distinguishes a "Clicked" from a "DoubleClicked". A
 * Clicked turns on the LED. A DoubleClick turns off the LED.
 *
 * The current AceButton code cannot actually separate a Clicked from a
 * DoubleClicked because the Clicked event will always trigger if a
 * DoubleClicked occurs. We cannot suppress the first Clicked because it's
 * already happened in the past by the time the DoubleClicked occurs, and the
 * first Clicked cannot predict the future.
 *
 ...
 * the default values. AceButton does not currently implement this feature
 * because the delay seemed too long.
 *
 * The workaround is to use a Released event instead of a Clicked to trigger the
 ...
 * Pressed/Released input from a Clicked input. Often this is actually the what
 * we want, because users are not always physically able to perform the two
